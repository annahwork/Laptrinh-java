...
@RestController
@RequestMapping("/api/sc-staff/notifications")
public class NotificationController {
    ...
    public ResponseEntity<String> createNotification(
        ...
        if (success)
            ...
            return ResponseEntity.badRequest().body("Tạo thông báo thất bại (User không tồn tại?).");
    }

    @GetMapping("/user")
    public ResponseEntity<List<Notification>> getAllUserNotifications(HttpSession session,
            @RequestParam Integer userID) {
    ...
    public ResponseEntity<String> updateNotification(HttpSession session, @PathVariable Integer id,
        ...
        if (success)
            ...
            return ResponseEntity.status(404).body("Không tìm thấy thông báo để sửa.");
    }
    ...
    public ResponseEntity<String> markAsRead(HttpSession session, @PathVariable Integer id) {
        ...
        if (success)
            return ResponseEntity.ok("Đánh dấu đã đọc!");
        else
            return ResponseEntity.status(404).body("Không tìm thấy thông báo.");
    }
    ...
    public ResponseEntity<String> markAllAsRead(HttpSession session, @PathVariable Integer userId) {
        ...
        if (success)
            return ResponseEntity.ok("Đã đánh dấu tất cả!");
        else
