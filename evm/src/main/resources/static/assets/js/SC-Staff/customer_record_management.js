(function () {
  ...
  async function loadCustomers(page = 1, pageSize = 1000, search = '') {
    try {
      const res = await fetch(`/evm/api/sc-staff/dashboard/customers?page=${page}&pageSize=${pageSize}`);
  ...
  async function deleteCustomer(id) {
    try {
      const res = await fetch(`/evm/api/sc-staff/dashboard/customer/delete/${id}`, { method: 'DELETE' });
      if (!res.ok) {
        const text = await res.text().catch(() => '');
        throw new Error(text || `Server trả về ${res.status}`);
      }
